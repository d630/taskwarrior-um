...
taskwarrior-um - Use Taskwarrior to maintain a list of urimarks aka. bookmarks.
...
very nice command line interface to maintain a list of tasks and to act like a
...
wraps up Taskwarrior and sees to it that only new URIs will have been recorded.
...
irrelevant, if a description conforms with a another description of different
task. Then, to identify a task there are the UUID field as permanent key and the
[id](http://taskwarrior.org/docs/ids.html) field as temporary assignment (line counted relative to the entries); not
until a task is completed or deleted, the id will have been discarded. The first
difference in using Taskwarrior as urimark manager is that there is - of
...
built-in command "read" of your bash prompts you to build the command arguments.
...
could look like this:
    ...
    taskum add uri:"http://taskwarrior.org/docs" name:taskwarrior note:"some
    tutorials and introductions" depends:1 pro:software.internet.bookmarking
    +taskwarrior +documentation +cms
...
-   Taskwarrior stores its data in json format. To export and convert your data is
    quite easy.

# ENVIROMENT

    To use Taskwarrior with non-default data dir and conf file 'TASKDATA' and
    'TASKRC' will be set. To build them automatically, we use our own variables:
...
'taskwarrior-um' is licensed with 'GNU GPLv3'. You should have received a copy
of the 'GNU General Public License' along with this program. If not, see for
more details '<http://www.gnu.org/licenses/gpl-3.0.html>'.

# CHRONOLOGY

First version ('0.1.0.0') was finished on: 21. April 2014.

# INTERACTIONS

## Firefox

To add the newest history in firefox to **taskwarrior-um** see:
<https://gist.github.com/D630/11103711>

## Mutt

To add an URI in Mutt:

    macro index,pager,attach,compose \Cb "\
    <enter-command> set my_pipe_decode=\$pipe_decode pipe_decode<Enter>\
    <pipe-message> /home/user/bin/mutt2taskum_bookmarks.sh<Enter>\
    <enter-command> set pipe_decode=\$my_pipe_decode; unset my_pipe_decode<Enter>" \
    "call mutt2taskum to extract URLs out of a message"

<https://gist.github.com/D630/11155240>

## Newsbeuter

To add an URI in Newsbeuter:

    bookmark-cmd "/home/user/bin/newsbeuter2taskum_bookmarks.sh"

<https://gist.github.com/D630/11155192>

## Urlview2

To pipe a report into urlview2:

    $ taskum | urlview2

<https://gist.github.com/D630/11247824>
